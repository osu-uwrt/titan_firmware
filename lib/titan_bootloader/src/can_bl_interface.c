...
MCP251XFD_Config mcp251xfd_device_config = {
    ...
    .SysInterruptFlags = MCP251XFD_INT_RX_EVENT  // Enable global RX interrupts (controlled via FIFO)
                                                 // No need to handle any other interrupts, if CAN goes down, the
                                                 // bootloader will timeout and the device will reset
};
...
MCP251XFD_FIFO mcp251xfd_utility_tx_fifo_config = {
    ...
    .Name = mcp251xfd_utility_tx_fifo,
...
MCP251XFD_Filter mcp251xfd_utility_rx_filter = {
    ...
    .AcceptanceID = 0,
    .AcceptanceMask = CANMORE_CALC_FILTER_MASK(true, true, true, true),
...
//  b) or if you just want to use the generic CAN_BUS_CLIENT_ID, that you don't set
//  TITAN_BOOTLOADER_CUSTOM_CLIENT_LOOKUP
