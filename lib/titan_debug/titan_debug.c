...
static bool fault_name_idx_cb(const struct reg_mapped_server_register_definition *reg, bool is_write,
                              uint32_t *data_ptr);
...
static const reg_mapped_server_register_def_t debug_server_safety_status_regs[] = {
    ...
    DEFINE_REG_EXEC_CALLBACK(CANMORE_DBG_SAFETY_STATUS_FAULT_NAME_IDX_OFFSET, fault_name_idx_cb,
...
static bool fault_name_idx_cb(__unused const struct reg_mapped_server_register_definition *reg, bool is_write,
                              uint32_t *data_ptr) {
    if (!is_write) {
