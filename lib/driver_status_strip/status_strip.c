...
static struct status_strip_inst {
    ...
    struct {
        uint32_t len;
        const uint32_t *data;
    } control_blocks[4];  // 3 commands: start, middle, and end, and 1 for stop
    ...
    // Flash State Config
    volatile bool
        flash_active;  // Voltaile as this is used to protect the flashing state, rather than disabling interrupts
    uint flash_timer;
    uint flash_count;
    ...
    if (inst->startup_flash_phase < STARTUP_PHASE_DONE) {
        ...
        if (inst->timer >= STARTUP_TICK_DURATION) {
            ...
            if (inst->startup_flash_phase == STARTUP_PHASE_GROW || inst->startup_flash_phase == STARTUP_PHASE_SHRINK) {
                ...
                // b) We are at the second phase, meaning the front should be colored, so the first pixel should be in
                // the rear
...
void status_strip_init(PIO pio, uint sm, uint pin, bool first_pixel_is_rear) {
    ...
    dma_channel_configure(inst->dma_ctrl_chan, &c,
    ...
    dma_channel_configure(inst->dma_data_chan, &c, &pio->txf[sm],
