...
static void sht41_on_error(const sht41_error_code error_type, uint32_t i2c_error_code);
...
static void sht41_on_error(const sht41_error_code error_type, uint32_t i2c_error_code) {
    ...
    if (err_count >= 3) {
        if (error_cb) {
            error_cb(error_type, i2c_error_code);
        }
    }
}

// called by the i2c
static void on_async_i2c_error(__unused struct async_i2c_request *req, uint32_t error_code) {
