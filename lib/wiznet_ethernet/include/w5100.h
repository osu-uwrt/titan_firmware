...
static void w5100_write##name(w5k_data_t _c, uint16_t _data) {            \
    uint8_t buf[2];                                                       \
    buf[0] = _data >> 8;                                                  \
    buf[1] = _data & 0xFF;                                                \
    w5100_write(_c, address, buf, 2);                                     \
}                                                                         \
static uint16_t w5100_read##name(w5k_data_t _c) {                         \
    uint8_t buf[2];                                                       \
    w5100_read(_c, address, buf, 2);                                      \
    return (buf[0] << 8) | buf[1];                                        \
}
#define __GP_REGISTER_N(name, address, size)                              \
static uint16_t w5100_write##name(w5k_data_t _c, const uint8_t *_buff) {  \
    return w5100_write(_c, address, _buff, size);                         \
}                                                                         \
static uint16_t w5100_read##name(w5k_data_t _c, uint8_t *_buff) {         \
