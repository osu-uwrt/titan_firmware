...
SOCKET eth_socketCreate(w5k_data_t *c, uint8_t *ip, uint16_t sport, uint8_t protocol, uint16_t dport,
                        uint8_t *state_ref);
...
typedef struct udp_socket {
    ...
    uint8_t socket_active;  // If the socket is active on the device (Cleared by lower level logic). Note: if this is
                            // zero after init, w5k is no longer valid
...
static inline uint8_t *eth_udp_remoteIP(udp_socket_t *s) {
    return s->remoteIP;
}
static inline uint16_t eth_udp_remotePort(udp_socket_t *s) {
    return s->remotePort;
}
