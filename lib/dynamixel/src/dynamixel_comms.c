...
static void on_receive_reply_header(enum async_uart_rx_err error, uint8_t *data, size_t data_len) {
    ...
    async_uart_read(rs485_buf + DYNAMIXEL_REPLY_HEADER_SIZE, body_length, on_receive_reply_body);
}
...
static void on_packet_sent(enum async_uart_tx_err error) {
    if(error) {
        ...
        async_uart_read(rs485_buf, DYNAMIXEL_REPLY_HEADER_SIZE, on_receive_reply_header);
    }
}
