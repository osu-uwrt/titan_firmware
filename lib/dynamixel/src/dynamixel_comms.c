...
#define dynamixel_comms_report_error_with_arg(error_code, arg) do { \
    dynamixel_error_t error_msg = {.fields = {.error = error_code, .error_source = DYNAMIXEL_SOURCE_COMMS, \
...
#define check_transfer_dxl_call(ret_code, expected) do {\
...
static void on_receive_reply_body(enum async_uart_rx_err error, uint8_t *data, size_t data_len) {
  ...
  dynamixel_error_t errcode = {.fields = {.error = DYNAMIXEL_ERROR_NONE, .line = __LINE__, .error_source = DYNAMIXEL_SOURCE_COMMS}};
