...
#define uart_multidrop_irq_tx_compl(sm) ((pis_interrupt0 + sm + uart_multidrop_irq_tx_compl_rel) % 4)
#define uart_multidrop_irq_frame_error(sm) ((pis_interrupt0 + sm + uart_multidrop_irq_frame_error_rel) % 4)
...
bool __time_critical_func(async_uart_check_and_finish_rx)(enum async_uart_rx_err error_code, bool report_if_fifoover) {
    ...
    uint32_t overflow_flag = (1u << PIO_FDEBUG_TXOVER_LSB) << inst->sm;
...
void __time_critical_func(async_uart_dma_handler)(void) {
    dma_irqn_acknowledge_channel(ASYNC_UART_DMA_IRQ, inst->dma_rx_chan);
    async_uart_check_and_finish_rx(ASYNC_UART_RX_OK, true);
}

void __time_critical_func(async_uart_pio_handler)(void) {
    if (pio_interrupt_get(inst->pio, uart_multidrop_irq_tx_compl(inst->sm))) {
        pio_interrupt_clear(inst->pio, uart_multidrop_irq_tx_compl(inst->sm));

        // No need for critical section, we're the only ones clearing this
        if (inst->tx_active) {
            inst->tx_active = false;
            inst->tx_cb(ASYNC_UART_TX_OK);
        }
    }

    if (pio_interrupt_get(inst->pio, uart_multidrop_irq_frame_error(inst->sm))) {
        pio_interrupt_clear(inst->pio, uart_multidrop_irq_frame_error(inst->sm));
...
void async_uart_init(PIO pio, unsigned int sm, unsigned int pin, unsigned int baud, unsigned int timeout_ms) {
    ...
    dma_irqn_set_channel_enabled(ASYNC_UART_DMA_IRQ, inst->dma_rx_chan, true);
    irq_set_exclusive_handler(ASYNC_UART_DMA_IRQ, async_uart_dma_handler);

    // Configure IRQs for PIO
    pio_interrupt_clear(pio, uart_multidrop_irq_tx_compl(sm));
...
void async_uart_write(const uint8_t *data, size_t len, bool preserve_read, async_uart_on_write cb) {
    ...
    // FIFO should be empty because tx_active
...
void async_uart_read(uint8_t *data, size_t len, async_uart_on_read cb) {
    ...
    // Begin transfer
    dma_hw->ch[inst->dma_tx_chan].al1_write_addr = (uintptr_t) data;
    dma_hw->ch[inst->dma_tx_chan].al1_transfer_count_trig = len;
