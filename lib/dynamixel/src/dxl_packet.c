...
static DXLLibErrorCode_t
add_param_to_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet,
                           uint8_t *p_param, uint16_t param_len);

static DXLLibErrorCode_t
end_make_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet);
static DXLLibErrorCode_t
end_make_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet);
...
DXLLibErrorCode_t begin_make_dxl_packet(InfoToMakeDXLPacket_t *p_make_packet,
                                        uint8_t id, uint8_t protocol_ver,
                                        uint8_t inst_idx, uint8_t err_idx,
  ...
  if (protocol_ver == 2) {
    if (id == 0xFD &&
        id ==
            0xFF) { // http://emanual.robotis.com/docs/en/dxl/protocol2/#packet-id
      return DXL_LIB_ERROR_INVAILD_ID;
    }
    if (packet_buf_capacity <
        11) { // http://emanual.robotis.com/docs/en/dxl/protocol2/#status-packet
      return DXL_LIB_ERROR_NOT_ENOUGH_BUFFER_SIZE;
    }
  } else if (protocol_ver == 1) {
    if (id ==
        0xFF) { // http://emanual.robotis.com/docs/en/dxl/protocol1/#packet-id
      return DXL_LIB_ERROR_INVAILD_ID;
    }
    if (packet_buf_capacity <
        6) { // http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
      return DXL_LIB_ERROR_NOT_ENOUGH_BUFFER_SIZE;
    }
  } else {
...
add_param_to_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet,
  ...
  if (p_make_packet->packet_buf_capacity <
          generated_param_len + param_len +
              6 // 6 = Header(2)+ID(1)+Length(1)+Instruction(1)+CheckSum(1) :
                // http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
      ||
      generated_param_len + param_len >
          255 - 2) { // Length Frame Range(1byte) =
...
add_param_to_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet,
  ...
  if (p_make_packet->inst_idx == DXL_INST_STATUS) {
    if (p_make_packet->packet_buf_capacity <
        generated_param_len + param_len + 11) {
      return DXL_LIB_ERROR_BUFFER_OVERFLOW;
    }
    ...
    if (p_make_packet->packet_buf_capacity <
        generated_param_len + param_len + 10) {
      return DXL_LIB_ERROR_BUFFER_OVERFLOW;
    }
...
end_make_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet) {
  ...
  p_packet[generated_packet_len++] =
      p_make_packet->param_length + 2; // 2 = Instruction(1)+CheckSum(1)
  if (p_make_packet->inst_idx == DXL_INST_STATUS) {
...
end_make_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet) {
  ...
  if (p_make_packet->inst_idx == DXL_INST_STATUS &&
    ...
    return DXL_LIB_ERROR_BUFFER_OVERFLOW;
  } else if (p_make_packet->packet_buf_capacity <
             p_make_packet->param_length + 10) {
    return DXL_LIB_ERROR_BUFFER_OVERFLOW;
  }
  ...
  p_packet[generated_packet_len++] = p_make_packet->id;
  if (p_make_packet->inst_idx == DXL_INST_STATUS) {
    p_packet[generated_packet_len++] = (p_make_packet->param_length + 4) >>
                                       0; // 4 = Instruction(1)+Error(1)+CRC(2)
    p_packet[generated_packet_len++] = (p_make_packet->param_length + 4) >>
                                       8; // 4 = Instruction(1)+Error(1)+CRC(2)
    p_packet[generated_packet_len++] = p_make_packet->inst_idx;
    p_packet[generated_packet_len++] = p_make_packet->err_idx;
  } else {
    p_packet[generated_packet_len++] =
        (p_make_packet->param_length + 3) >> 0; // 3 = Instruction(1)+CRC(2)
    p_packet[generated_packet_len++] =
        (p_make_packet->param_length + 3) >> 8; // 3 = Instruction(1)+CRC(2)
    p_packet[generated_packet_len++] = p_make_packet->inst_idx;
  }
  generated_packet_len += p_make_packet->param_length;
...
DXLLibErrorCode_t parse_dxl_packet(InfoToParseDXLPacket_t *p_parse_packet,
                                   uint8_t recv_data) {
  DXLLibErrorCode_t ret;
...
parse_dxl1_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data) {
  ...
  case DXL1_0_PACKET_PARSING_STATE_LENGTH:
    if ((uint16_t)recv_data > p_parse_packet->param_buf_capacity +
                                  2) { // 2 = Instruction(1) + CheckSum(1)
  ...
  case DXL1_0_PACKET_PARSING_STATE_PARAM:
    ...
    if (p_parse_packet->recv_param_len ==
        p_parse_packet->packet_len - 2) { // 2 = Instruction(1) + CheckSum(1)
      p_parse_packet->parse_state = DXL1_0_PACKET_PARSING_STATE_CHECK_SUM;
    }
    break;

  case DXL1_0_PACKET_PARSING_STATE_CHECK_SUM:
    p_parse_packet->calculated_check_sum =
        ~(p_parse_packet->calculated_check_sum);
    p_parse_packet->recv_check_sum = recv_data;
    if (p_parse_packet->calculated_check_sum ==
        p_parse_packet->recv_check_sum) {
      ret = DXL_LIB_OK;
    } else {
...
parse_dxl2_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data) {
  ...
  case DXL2_0_PACKET_PARSING_STATE_RESERVED:
    if (recv_data !=
        0xFD) { // http://emanual.robotis.com/docs/en/dxl/protocol2/#reserved
  ...
  case DXL2_0_PACKET_PARSING_STATE_ID:
    if (recv_data < 0xFD ||
        recv_data ==
            DXL_BROADCAST_ID) { // http://emanual.robotis.com/docs/en/dxl/protocol2/#packet-id
      p_parse_packet->id = recv_data;
      update_dxl_crc(&p_parse_packet->calculated_crc, recv_data);
  ...
  case DXL2_0_PACKET_PARSING_STATE_INST:
    ...
    if (recv_data == DXL_INST_STATUS) {
      if (p_parse_packet->packet_len <
          4) { // 4 = Instruction(1)+Error(1)+CRC(2)
        ret = DXL_LIB_ERROR_LENGTH;
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
      ...
      if (p_parse_packet->packet_len >
          p_parse_packet->param_buf_capacity + 3) { // 3 = Instruction(1)+CRC(2)
        ret = DXL_LIB_ERROR_BUFFER_OVERFLOW;
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
  ...
  case DXL2_0_PACKET_PARSING_STATE_PARAM:
    ...
    if (p_parse_packet->inst_idx == DXL_INST_STATUS) {
      if (p_parse_packet->recv_param_len >= 4) {
        if (p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 4] ==
                0xFF &&
            p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 3] ==
                0xFF &&
            p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 2] ==
                0xFD &&
            p_parse_packet->p_param_buf[p_parse_packet->recv_param_len - 1] ==
                0xFD) {
...
fast_parse_dxl2_0_packet(InfoToParseDXLPacket_t *p_parse_packet,
  ...
  uint16_t byte_stuffing_cnt = 0;
  uint8_t size =
      ((p_parse_packet->param_buf_capacity + 4) * p_parse_packet->xel_count) -
      3; // 4 = Instruction(1)+Error(1)+CRC(2)
    ...
    if (recv_data == DXL_INST_STATUS) {
      ...
      if (p_parse_packet->packet_len <
        ...
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
                 ...
                 ((p_parse_packet->param_buf_capacity + 4) *
                  p_parse_packet->xel_count) +
                     1) { // 4 = Instruction(1)+Error(1)+CRC(2)
        ret = DXL_LIB_ERROR_BUFFER_OVERFLOW;
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
      } else {
        p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_ERROR;
  ...
  case DXL2_0_PACKET_PARSING_STATE_PARAM:
    ...
    for (p_parse_packet->check_xel_count = 0;
         p_parse_packet->check_xel_count < p_parse_packet->xel_count;
         p_parse_packet->check_xel_count++) {
      if (p_parse_packet->check_xel_count == 0) {
        ...
        if ((p_parse_packet->check_xel_count *
             (p_parse_packet->param_buf_capacity + 4)) <
                p_parse_packet->recv_param_len &&
            p_parse_packet->recv_param_len <=
                ((p_parse_packet->check_xel_count + 1) *
                 p_parse_packet->param_buf_capacity) +
                    4) {
          p_parse_packet->p_param_buf[p_parse_packet->param_count] = recv_data;
          p_parse_packet->param_count += 1;
