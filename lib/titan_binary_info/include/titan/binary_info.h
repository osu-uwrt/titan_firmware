...
#define bi_device_ip_address_array(address)                                                                            \
    bi_int(BINARY_INFO_TAG_UWRT, BINARY_INFO_ID_UW_DEVICE_IP_ADDRESS, bi_ip_address_array(address))
#define bi_agent_ip_address_array(address)                                                                             \
    bi_int(BINARY_INFO_TAG_UWRT, BINARY_INFO_ID_UW_AGENT_IP_ADDRESS, bi_ip_address_array(address))
#define bi_agent_port(port) bi_int(BINARY_INFO_TAG_UWRT, BINARY_INFO_ID_UW_AGENT_PORT, port)
#define bi_titan_version(major, minor, type)                                                                           \
    bi_int(BINARY_INFO_TAG_UWRT, BINARY_INFO_ID_UW_VERSION_DATA,                                                       \
           ((major & 0xFF) << 16) | ((major & 0xFF) << 8) | (type & 0xFF))
