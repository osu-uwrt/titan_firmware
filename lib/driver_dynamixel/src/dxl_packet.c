...
static DXLLibErrorCode_t add_param_to_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param,
                                                    uint16_t param_len);
static DXLLibErrorCode_t add_param_to_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param,
                                                    uint16_t param_len);
...
//-- External Functions
//
DXLLibErrorCode_t begin_make_dxl_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t id, uint8_t protocol_ver,
                                        uint8_t inst_idx, uint8_t err_idx, uint8_t *p_packet_buf,
...
static DXLLibErrorCode_t add_param_to_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param,
    ...
    if (p_make_packet->packet_buf_capacity <
            generated_param_len + param_len + 6  // 6 = Header(2)+ID(1)+Length(1)+Instruction(1)+CheckSum(1) :
                                                 // http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
        || generated_param_len + param_len > 255 - 2) {  // Length Frame Range(1byte) =
                                                         // Instruction(1)+Param(N)+CheckSum(1) :
                                                         // http://emanual.robotis.com/docs/en/dxl/protocol1/#length
...
static DXLLibErrorCode_t add_param_to_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet, uint8_t *p_param,
    ...
    if (p_make_packet->inst_idx == DXL_INST_STATUS) {
        ...
        for (i = 0; i < param_len; i++) {
            ...
            // Add byte stuffing
            // (http://emanual.robotis.com/docs/en/dxl/protocol2/#processing-order-of-transmission)
            if (generated_param_len >= 3) {
                if (p_param_buf[generated_param_len - 3] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF &&
                    p_param_buf[generated_param_len - 1] == 0xFD) {
                    p_param_buf[generated_param_len++] = 0xFD;
                }
            }
            else if (generated_param_len == 2) {
                if (p_packet[DXL2_0_PACKET_IDX_ERROR] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF &&
                    p_param_buf[generated_param_len - 1] == 0xFD) {
                    p_param_buf[generated_param_len++] = 0xFD;
                }
            }
            else if (generated_param_len == 1) {
                if (p_packet[DXL2_0_PACKET_IDX_INST] == 0xFF && p_packet[DXL2_0_PACKET_IDX_ERROR] == 0xFF &&
                    p_param_buf[generated_param_len - 1] == 0xFD) {
                    p_param_buf[generated_param_len++] = 0xFD;
                }
            }
        ...
        for (i = 0; i < param_len; i++) {
            ...
            if (generated_param_len >= 3) {
                ...
                if (p_packet[DXL2_0_PACKET_IDX_INST] == 0xFF && p_param_buf[generated_param_len - 2] == 0xFF &&
                    p_param_buf[generated_param_len - 1] == 0xFD) {
                    p_param_buf[generated_param_len++] = 0xFD;
                }
            }
...
static DXLLibErrorCode_t end_make_dxl1_0_packet(InfoToMakeDXLPacket_t *p_make_packet) {
    ...
    if (p_make_packet->packet_buf_capacity <
            p_make_packet->param_length + 6  // 6 = Header(2)+ID(1)+Length(1)+Instruction(1)+CheckSum(1) :
                                             // http://emanual.robotis.com/docs/en/dxl/protocol1/#instruction-packet
        || p_make_packet->param_length > 255 - 2) {  // Length Frame Range(1byte) =
                                                     // Instruction(1)+Param(N)+CheckSum(1) :
                                                     // http://emanual.robotis.com/docs/en/dxl/protocol1/#length
        return DXL_LIB_ERROR_BUFFER_OVERFLOW;
    }
...
static DXLLibErrorCode_t end_make_dxl2_0_packet(InfoToMakeDXLPacket_t *p_make_packet) {
    ...
    if (p_make_packet->inst_idx == DXL_INST_STATUS &&
        p_make_packet->packet_buf_capacity < p_make_packet->param_length + 11) {
        return DXL_LIB_ERROR_BUFFER_OVERFLOW;
    }
    ...
    p_packet[generated_packet_len++] = p_make_packet->id;
    if (p_make_packet->inst_idx == DXL_INST_STATUS) {
        p_packet[generated_packet_len++] =
            (p_make_packet->param_length + 4) >> 0;  // 4 = Instruction(1)+Error(1)+CRC(2)
        p_packet[generated_packet_len++] =
            (p_make_packet->param_length + 4) >> 8;  // 4 = Instruction(1)+Error(1)+CRC(2)
        p_packet[generated_packet_len++] = p_make_packet->inst_idx;
        p_packet[generated_packet_len++] = p_make_packet->err_idx;
    }
    else {
...
DXLLibErrorCode_t begin_parse_dxl_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t protocol_ver,
                                         uint8_t *p_param_buf, uint16_t param_buf_capacity) {
...
// DXLLibErrorCode_t
// fast_begin_parse_dxl_packet(InfoToParseDXLPacket_t *p_parse_packet,
//                             uint8_t protocol_ver, uint8_t *p_param_buf,
//                             uint16_t param_buf_capacity, uint8_t xel_count) {
//   if (param_buf_capacity > 0 && p_param_buf == NULL) {
//     return DXL_LIB_ERROR_NULLPTR;
//   }
...
// DXLLibErrorCode_t fast_parse_dxl_packet(InfoToParseDXLPacket_t *p_parse_packet,
//                                         uint8_t recv_data) {
//   DXLLibErrorCode_t ret;
...
static DXLLibErrorCode_t parse_dxl1_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data) {
    ...
    case DXL1_0_PACKET_PARSING_STATE_IDLE:
        ...
        if (p_parse_packet->header_cnt == 2) {
            if (p_parse_packet->header[0] == 0xFF && p_parse_packet->header[1] == 0xFF) {
                p_parse_packet->recv_param_len = 0;
...
static DXLLibErrorCode_t parse_dxl2_0_packet(InfoToParseDXLPacket_t *p_parse_packet, uint8_t recv_data) {
    ...
    case DXL2_0_PACKET_PARSING_STATE_ID:
        if (recv_data < 0xFD ||
            recv_data == DXL_BROADCAST_ID) {  // http://emanual.robotis.com/docs/en/dxl/protocol2/#packet-id
            p_parse_packet->id = recv_data;
            update_dxl_crc(&p_parse_packet->calculated_crc, recv_data);
        ...
        if (recv_data == DXL_INST_STATUS) {
            if (p_parse_packet->packet_len < 4) {  // 4 = Instruction(1)+Error(1)+CRC(2)
                ...
                p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
                     ...
                     p_parse_packet->param_buf_capacity + 4) {  // 4 = Instruction(1)+Error(1)+CRC(2)
                ret = DXL_LIB_ERROR_BUFFER_OVERFLOW;
                p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
            }
            else {
    ...
    case DXL2_0_PACKET_PARSING_STATE_PARAM:
        ...
        // (http://emanual.robotis.com/docs/en/dxl/protocol2/#processing-order-of-reception)
        if (p_parse_packet->inst_idx == DXL_INST_STATUS) {
            if (p_parse_packet->recv_param_len >= 4) {
                ...
                if (p_parse_packet->err_idx == 0xFF && p_parse_packet->p_param_buf[0] == 0xFF &&
                    p_parse_packet->p_param_buf[1] == 0xFD && p_parse_packet->p_param_buf[2] == 0xFD) {
                    p_parse_packet->recv_param_len--;
                    byte_stuffing_cnt++;
                }
            }
            else if (p_parse_packet->recv_param_len == 2) {
                if (p_parse_packet->inst_idx == 0xFF && p_parse_packet->err_idx == 0xFF &&
                    p_parse_packet->p_param_buf[0] == 0xFD && p_parse_packet->p_param_buf[1] == 0xFD) {
                    p_parse_packet->recv_param_len--;
                    byte_stuffing_cnt++;
                }
            }
            if (p_parse_packet->recv_param_len + byte_stuffing_cnt + 4 ==
                p_parse_packet->packet_len) {  // 4 = Instruction(1)+Error(1)+CRC(2)
                p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_CRC_L;
            }
        }
        else {
            if (p_parse_packet->recv_param_len >= 4) {
                ...
                if (p_parse_packet->inst_idx == 0xFF && p_parse_packet->p_param_buf[0] == 0xFF &&
                    p_parse_packet->p_param_buf[1] == 0xFD && p_parse_packet->p_param_buf[2] == 0xFD) {
                    p_parse_packet->recv_param_len--;
                    byte_stuffing_cnt++;
                }
            }
            if (p_parse_packet->recv_param_len + byte_stuffing_cnt + 3 ==
                p_parse_packet->packet_len) {  // 3 = Instruction(1)+CRC(2)
                p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_CRC_L;
            }
        }
        break;
...
/*static DXLLibErrorCode_t fast_parse_dxl2_0_packet(InfoToParseDXLPacket_t* p_parse_packet, uint8_t recv_data)
{
  DXLLibErrorCode_t ret = DXL_LIB_PROCEEDING;
  uint16_t byte_stuffing_cnt = 0;
  uint8_t size = ((p_parse_packet->param_buf_capacity+4) * p_parse_packet->xel_count) - 3; // 4 =
Instruction(1)+Error(1)+CRC(2) uint8_t* param_array = (uint8_t *)malloc(sizeof(uint8_t) * size);
  switch(p_parse_packet->parse_state)
    {
      case DXL2_0_PACKET_PARSING_STATE_IDLE:
        if(p_parse_packet->header_cnt >= 3){
      ...
      case DXL2_0_PACKET_PARSING_STATE_RESERVED:
        ...
        break;
      case DXL2_0_PACKET_PARSING_STATE_ID:
        if(recv_data < 0xFD || recv_data == DXL_BROADCAST_ID){
//http://emanual.robotis.com/docs/en/dxl/protocol2/#packet-id p_parse_packet->id = recv_data;
          update_dxl_crc(&p_parse_packet->calculated_crc, recv_data);
          p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_LENGTH_L;
        }else{
          ret = DXL_LIB_ERROR_INVAILD_ID;
      ...
      case DXL2_0_PACKET_PARSING_STATE_INST:
        ...
        if(recv_data == DXL_INST_STATUS){
          ...
          if(p_parse_packet->packet_len < 4){ // 4 = Instruction(1)+Error(1)+CRC(2)
            ret = DXL_LIB_ERROR_LENGTH;
            p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_IDLE;
          }else if(p_parse_packet->packet_len > ((p_parse_packet->param_buf_capacity+4) * p_parse_packet->xel_count) + 1
){ // 4 = Instruction(1)+Error(1)+CRC(2) ret = DXL_LIB_ERROR_BUFFER_OVERFLOW; p_parse_packet->parse_state =
DXL2_0_PACKET_PARSING_STATE_IDLE; }else{ p_parse_packet->parse_state = DXL2_0_PACKET_PARSING_STATE_ERROR;
          }
        }else{
          if(p_parse_packet->packet_len > p_parse_packet->param_buf_capacity+3){ // 3 = Instruction(1)+CRC(2)
            ret = DXL_LIB_ERROR_BUFFER_OVERFLOW;
      ...
      case DXL2_0_PACKET_PARSING_STATE_PARAM:
        ...
        for(p_parse_packet->check_xel_count=0; p_parse_packet->check_xel_count < p_parse_packet->xel_count;
p_parse_packet->check_xel_count++){ if(p_parse_packet->check_xel_count == 0) { if(0 < p_parse_packet->recv_param_len &&
p_parse_packet->recv_param_len <= p_parse_packet->param_buf_capacity) {
              p_parse_packet->p_param_buf[p_parse_packet->param_count] = recv_data;
              p_parse_packet->param_count += 1;
            }
          } else {
            if((p_parse_packet->check_xel_count * (p_parse_packet->param_buf_capacity + 4)) <
p_parse_packet->recv_param_len
                && p_parse_packet->recv_param_len <= ((p_parse_packet->check_xel_count + 1) *
p_parse_packet->param_buf_capacity) + 4) { p_parse_packet->p_param_buf[p_parse_packet->param_count] = recv_data;
              p_parse_packet->param_count += 1;
            }
          }
        }
...
const unsigned short crc_table[256] PROGMEM = {
