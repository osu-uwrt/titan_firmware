...
BootloaderClient::BootloaderClient(std::shared_ptr<RegMappedClient> client): client(client), erasedSectors(0), writtenPages(0) {
    client->configureInterfaceMode(CANMORE_TITAN_CONTROL_INTERFACE_MODE_BOOTLOADER);

    // Make sure we're talking to what we expect to
    uint32_t bl_magic = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_MAGIC_OFFSET);
    ...
    uint32_t version_major = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_MAJOR_VERSION_OFFSET);
    uint32_t version_minor = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_MINOR_VERSION_OFFSET);
    uint32_t version_type = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_RELEASE_TYPE_OFFSET);
    ...
    cachedFlashID.word[0] = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_LOWER_FLASH_ID);
    cachedFlashID.word[1] = client->readRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_UPPER_FLASH_ID);
    cachedFlashSize = client->readRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_FLASH_SIZE_OFFSET);

    // Disable bootloader overwrite in case it was enabled previously
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_BL_WRITE_KEY_OFFSET, 0);
}

void BootloaderClient::getVersion(std::string &versionOut) {
    client->readStringPage(CANMORE_BL_VERSION_STRING_PAGE_NUM, versionOut);
}
...
void BootloaderClient::readBytes(uint32_t addr, std::vector<uint8_t> &bytesOut) {
    ...
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_TARGET_ADDR_OFFSET, addr);
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_COMMAND_OFFSET, CANMORE_BL_FLASH_CONTROL_COMMAND_READ);
    client->readArray(CANMORE_BL_FLASH_BUFFER_PAGE_NUM, 0, flashContents, CANMORE_BL_FLASH_BUFFER_SIZE/sizeof(uint32_t));
...
void BootloaderClient::writeBytes(uint32_t addr, std::vector<uint8_t> &bytes) {
    ...
    if (!erasedSectors.test(sectorNum)) {
        ...
        client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_TARGET_ADDR_OFFSET, addr);
        client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_COMMAND_OFFSET, CANMORE_BL_FLASH_CONTROL_COMMAND_ERASE);
    }

    // Write the data
    client->writeArray(CANMORE_BL_FLASH_BUFFER_PAGE_NUM, 0, flashContents);
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_TARGET_ADDR_OFFSET, addr);
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_COMMAND_OFFSET, CANMORE_BL_FLASH_CONTROL_COMMAND_WRITE);
}
...
bool BootloaderClient::verifyBytes(uint32_t addr, std::vector<uint8_t> &bytes) {
    ...
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_TARGET_ADDR_OFFSET, addr);
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_COMMAND_OFFSET, CANMORE_BL_FLASH_CONTROL_COMMAND_READ);
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_COMMAND_OFFSET, CANMORE_BL_FLASH_CONTROL_COMMAND_CRC);
    uint32_t crcRead = client->readRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_CRC_OFFSET);

    return crcExpected == crcRead;
}

void BootloaderClient::enableBootloaderOverwrite() {
    client->writeRegister(CANMORE_BL_FLASH_CONTROL_PAGE_NUM, CANMORE_BL_FLASH_CONTROL_BL_WRITE_KEY_OFFSET, CANMORE_BL_FLASH_CONTROL_BL_WRITE_KEY_VALUE);
}

void BootloaderClient::reboot() {
    client->writeRegister(CANMORE_BL_MCU_CONTROL_PAGE_NUM, CANMORE_BL_MCU_CONTROL_REBOOT_MCU_OFFSET, 1);
}