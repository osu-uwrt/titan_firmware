...
RegMappedCANClient::RegMappedCANClient(int ifIndex, uint8_t clientId, uint8_t channel, uint32_t timeoutMs) :
    ifIndex(ifIndex), clientId(clientId), channel(channel), socketFd(-1)
{
    ...
    if (flags < 0) {
        throw std::system_error(errno, std::generic_category(), "CAN fnctl F_GETFL");
    }
    if (fcntl(socketFd, F_SETFL, flags | O_NONBLOCK) < 0) {
        throw std::system_error(errno, std::generic_category(), "CAN fnctl F_SETFL");
    }
    ...
    clientCfg.arg = this;
    clientCfg.timeout_ms = timeoutMs;
}
