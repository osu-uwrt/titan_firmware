...
public:
    void callback(CLIInterface<Canmore::BootloaderClient> &interface) override {
        interface.handle->ping();
    }
};
...
public:
    ...
    void callbackSafe(CLIInterface<Canmore::BootloaderClient> &interface, std::vector<std::string> const& args) override {
        (void) args;
...
public:
    ...
    std::string getHelp() const override {return "Reads a page at the specified address\nAddress must be a flash address in hex";}
