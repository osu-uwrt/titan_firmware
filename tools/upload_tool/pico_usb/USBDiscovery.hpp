...
public:
    usb_error(const std::string& msg, libusb_error error_code)
        : Base(error_code, libusb_category(), msg) {}
};

static inline ssize_t checkLibusbErrFunc(const char* func, ssize_t err)
{
    if (err < 0) throw usb_error(func, static_cast<libusb_error>(err));
    return err;
}
...
class USBDeviceList {
public:
    USBDeviceList(libusb_context* ctx): _list(NULL), _idx(0) {_cnt = checkLibusbErr(libusb_get_device_list(ctx, &_list));}
    ~USBDeviceList() {if (_list) libusb_free_device_list(_list, 1);}

    bool hasNext() { return _idx < _cnt; }
    libusb_device* next() {
        if (_idx >= _cnt) throw std::out_of_range("USBDeviceList next");
        return _list[_idx++];
    }
...
public:
    USBDeviceHandle(libusb_device* dev, std::shared_ptr<USBDiscovery> discovery): handle(NULL), discoverySrc(discovery)
        {checkLibusbErr(libusb_open(dev, &handle));}
    ~USBDeviceHandle() {
        for (int itf : claimed_itfs) libusb_release_interface(handle, itf);
        if (handle) libusb_close(handle);
    }

    void claimInterface(int itf) {checkLibusbErr(libusb_claim_interface(handle, itf)); claimed_itfs.push_back(itf);}
...
class RP2040BootromInterface: public UploadTool::RP2040FlashInterface {
public:
...
public:
    UploadTool::RP2040Application firstApp, nestedApp;
};
...
class USBDevice: public UploadTool::RP2040Device {
public:
...
public:
    ...
    std::shared_ptr<UploadTool::RP2040FlashInterface> getFlashInterface() override
        {return std::static_pointer_cast<UploadTool::RP2040FlashInterface>(switchToBootromMode());}
...
public:
    ...
    void getAdditionalInfo(std::vector<std::pair<std::string, std::string>> &infoOut) override {
        ...
        UploadTool::reportVersionInfo(infoOut, itf->firstApp, itf->nestedApp);
    }
    ...
    std::shared_ptr<UploadTool::RP2040FlashInterface> getFlashInterface() override
        {return std::static_pointer_cast<UploadTool::RP2040FlashInterface>(getBootromInterface());}
private:
...
class USBDiscovery: public UploadTool::RP2040Discovery {
public:
    ~USBDiscovery();
