...
void connection::reset() {
    wrap_call([&] { return picoboot_reset(device, &itf); });
}

void connection::exclusive_access(uint8_t exclusive) {
    wrap_call([&] { return picoboot_exclusive_access(device, &itf, exclusive); });
}

void connection::enter_cmd_xip() {
    wrap_call([&] { return picoboot_enter_cmd_xip(device, &itf); });
}

void connection::exit_xip() {
    wrap_call([&] { return picoboot_exit_xip(device, &itf); });
}

void connection::reboot(uint32_t pc, uint32_t sp, uint32_t delay_ms) {
    wrap_call([&] { return picoboot_reboot(device, &itf, pc, sp, delay_ms); });
}

void connection::exec(uint32_t addr) {
    wrap_call([&] { return picoboot_exec(device, &itf, addr); });
}

void connection::flash_erase(uint32_t addr, uint32_t len) {
    wrap_call([&] { return picoboot_flash_erase(device, &itf, addr, len); });
}

void connection::vector(uint32_t addr) {
    wrap_call([&] { return picoboot_vector(device, &itf, addr); });
}

void connection::write(uint32_t addr, uint8_t *buffer, uint32_t len) {
    wrap_call([&] { return picoboot_write(device, &itf, addr, buffer, len); });
}

void connection::read(uint32_t addr, uint8_t *buffer, uint32_t len) {
    wrap_call([&] { return picoboot_read(device, &itf, addr, buffer, len); });
}
