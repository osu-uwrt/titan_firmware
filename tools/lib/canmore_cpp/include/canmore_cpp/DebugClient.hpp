...
struct SafetyStatus {
    ...
    SafetyStatus(uint32_t safetyStatusField):
        ...
        safetyInitialized((safetyStatusField & (1<<CANMORE_DBG_SAFETY_STATUS_GLOBAL_STATE_IS_INITIALIZED_FIELD)) != 0),
...
struct CrashCounter {
    ...
    CrashCounter(uint32_t crashCountField):
        totalCount((crashCountField >> CANMORE_DBG_CRASH_LOG_CRASH_COUNT_TOTAL_SHIFT) & CANMORE_DBG_CRASH_LOG_CRASH_COUNT_TOTAL_MASK),
        panicCount((crashCountField >> CANMORE_DBG_CRASH_LOG_CRASH_COUNT_PANIC_SHIFT) & CANMORE_DBG_CRASH_LOG_CRASH_COUNT_PANIC_MASK),
        hardFaultCount((crashCountField >> CANMORE_DBG_CRASH_LOG_CRASH_COUNT_HARD_FAULT_SHIFT) & CANMORE_DBG_CRASH_LOG_CRASH_COUNT_HARD_FAULT_MASK),
        assertFailCount((crashCountField >> CANMORE_DBG_CRASH_LOG_CRASH_COUNT_ASSERT_FAIL_SHIFT) & CANMORE_DBG_CRASH_LOG_CRASH_COUNT_ASSERT_FAIL_MASK) {}
...
struct CrashLogEntry {
    ...
    std::string format() const {
        ...
        if (resetReason == CANMORE_DBG_CRASH_LOG_RESET_REASON_CLEAN_BOOT_VALUE) {
            ...
            reasonStream << " - Faults: 0x" << faults << " - Uptime: " << std::setw(0) << std::setprecision(2) << uptime_pretty << uptime_units;
        }
