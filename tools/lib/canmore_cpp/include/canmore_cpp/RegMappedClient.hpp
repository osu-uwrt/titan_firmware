...
class RegMappedClientError : public CanmoreError
{
    public:
        RegMappedClientError(int errorCode, uint8_t mode, uint8_t page, uint8_t offset, bool isWrite):
            CanmoreError(std::string("Register Mapped Access Error (Register Direct ") + (isWrite ? "Write" : "Read") + " - mode: " + lookupMode(mode) +
        ...
        RegMappedClientError(int errorCode, uint8_t mode, uint8_t page, uint8_t offset, uint8_t length, bool isWrite):
            CanmoreError(std::string("Register Mapped Access Error (Array ") + (isWrite ? "Write" : "Read") + " - mode: " + lookupMode(mode) +
        ...
        RegMappedClientError(int errorCode, size_t length):
            CanmoreError("Raw Access Error (length: " + std::to_string(length) +
                         "): " + lookupError(errorCode)),
            errorCode(errorCode) {}
