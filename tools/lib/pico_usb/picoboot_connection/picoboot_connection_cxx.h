...
struct connection {
    explicit connection(libusb_device_handle *device, struct picoboot_itf itf, bool exclusive = true):
        device(device), itf(itf), exclusive(exclusive) {
    ...
    ~connection() {
        if (exclusive) {
            if (picoboot_exclusive_access(device, &itf, NOT_EXCLUSIVE)) {
                // failed to restore exclusive access, so just reset
                picoboot_reset(device, &itf);
            }
        }
    }
    void reset();
    ...
    void write_bytes(uint32_t addr, std::vector<uint8_t> bytes) { write(addr, bytes.data(), bytes.size()); }
