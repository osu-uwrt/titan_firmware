...
public:
    ...
    bool operator==(const Device &y) const { return (interfaceName == y.interfaceName) && (clientId == y.clientId); }
};

struct DeviceCmp {
    bool operator()(std::shared_ptr<Device> a, std::shared_ptr<Device> b) const { return *a < *b; }
};
...
public:
    ...
    std::shared_ptr<RP2040FlashInterface> getFlashInterface() override {
        return std::static_pointer_cast<RP2040FlashInterface>(switchToBootloaderMode());
    }
};
...
public:
    ...
    template <typename DeviceType> std::shared_ptr<DeviceType> waitForDevice(uint8_t clientId, int64_t timeoutMs) {
        ...
        for (std::shared_ptr<Device> d : discoveredDevices) {
            if (d->clientId == clientId && (device = std::dynamic_pointer_cast<DeviceType>(d)) != nullptr)
                return device;
        }
        ...
        // Note that this will return null if it timed out, as the dynamic cast is the last check before exiting the
        // wait condition
        return device;
    }
...
public:
    ...
    std::shared_ptr<DebugClient> getClient() override;
...
struct EthernetDiscoveryKey {
    ...
    bool operator==(const EthernetDiscoveryKey &other) const {
        (void) other;
        return true;
    }
};
...
struct EthernetDiscoveryKeyHasher {
    std::size_t operator()(const EthernetDiscoveryKey &k) const {
        (void) k;
...
class EthernetDiscovery :
    public Discovery,
    public SocketSingleton<EthernetDiscovery, EthernetDiscoveryKey, EthernetDiscoveryKeyHasher> {
public:
