...
//     // It should be fine to save this outside of a critical section, as this is only incremented by the firing
//     callback
...
//     // int32_t new_homing_offset = -1 * ((ram->present_position - homing_offset_compensation) - POSITION_HOME) %
//     4096;
...
//     // Actuator Arm
//     RCRETCHECK(rclc_subscription_init_default(&arm_subscription, node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg,
//     Bool),
//                                               ARM_SUBSCRIPTION_NAME));
//     RCRETCHECK(rclc_executor_add_subscription(executor, &arm_subscription, &actuator_arm_msg,
//     arm_subscription_callback,
//                                               ON_NEW_DATA));

//     // State Publishers
//     RCRETCHECK(rclc_publisher_init_best_effort(&busy_publisher, node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg,
//     Bool),
//                                                BUSY_TOPIC_NAME));
//     RCRETCHECK(rclc_publisher_init_best_effort(
//         &status_publisher, node, ROSIDL_GET_MSG_TYPE_SUPPORT(riptide_msgs2, msg, ActuatorStatus),
//         STATUS_TOPIC_NAME));
