...
bool ms5837_busy(unsigned int bus_id) {
    return depth_inst[bus_id].performing_operation;
}
...
void ms5837_init(unsigned int bus_id, enum depth_sensor_type sensor_type, ms5837_init_cb init_cb,
                 ms5837_error_cb error_cb) {
    struct depth_state *inst = &depth_inst[bus_id];

    // Make sure we aren't doing anything before we reinitialize state
    hard_assert_if(DEPTH, depth_inst[bus_id].performing_operation);
    ...
    LOG_INFO("Finished ms5837 setup");
 ...
 */
static void ms5837_reset_finished(const struct async_i2c_request *req) {
    LOG_INFO("ms5837 reset finished");
...
void ms5837_do_conversion(unsigned int bus_id, ms5837_read_cb read_cb, ms5837_error_cb error_cb) {
    struct depth_state *inst = &depth_inst[bus_id];

    // Make sure we aren't in an existing operation
    hard_assert_if(DEPTH, depth_inst[bus_id].performing_operation);
    hard_assert_if(DEPTH, !depth_inst[bus_id].sensor_initialized);
