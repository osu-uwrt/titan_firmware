...
void tx_init() {
    ...
    tx_ctrl.pwm_slice_num = pwm_gpio_to_slice_num(TX_PIN);
    uint pwm_chan = pwm_gpio_to_channel(TX_PIN);

    pwm_set_wrap(tx_ctrl.pwm_slice_num, PWM_WRAP_VALUE - 1);
    pwm_set_chan_level(tx_ctrl.pwm_slice_num, pwm_chan, PWM_WRAP_VALUE / 2);
...
void tx_encode_bit(uint8_t bit) {
    ...
    pwm_set_clkdiv(tx_ctrl.pwm_slice_num, clock_get_hz(clk_sys) / (freq * PWM_WRAP_VALUE));
}

void tx_encode_sync() {
    pwm_set_clkdiv(tx_ctrl.pwm_slice_num, clock_get_hz(clk_sys) / (FREQ_SYNC_HZ * PWM_WRAP_VALUE));
}

void tx_disable() {
    pwm_set_enabled(tx_ctrl.pwm_slice_num, false);
}
