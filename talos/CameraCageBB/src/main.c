...
#define LEAK_SENSE_PUBLISH_INTERVAL_MS 1000
...
absolute_time_t next_leak_sense_publish = { 0 };
...
static void start_ros_timers() {
    ...
    next_leak_sense_publish = make_timeout_time_ms(LEAK_SENSE_PUBLISH_INTERVAL_MS);
}
...
static void tick_ros_tasks() {
    ...
    if (timer_ready(&next_leak_sense_publish, LEAK_SENSE_PUBLISH_INTERVAL_MS, true)) {
        RCSOFTRETVCHECK(ros_update_leak_sense_publisher());
    }
}
