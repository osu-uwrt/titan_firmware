            ...
            if (retries < RETRANSMIT_COUNT) {
                ...
                while (multicore_fifo_rvalid()) {
                    ...
                    if (req.type == BQ_MAC_EMG_FET_CTRL_CMD) {
                        if (!local_opr_stat.discharge) {  // FIXME can't use old discharge flag
                        ...
                        // FIXME can't use old discharge flag
                        while (local_opr_stat.discharge && !time_reached(fet_off_time_end)) {
                            sleep_us(100);
                            watchdog_update();
                        }
...
static int bq_handle_i2c_transfer(uint8_t *bq_reg, uint8_t *rx_buf, uint len) {
    int ret_code = 0;  // if you set this to zero, the compiler *will delete* this function
...
static int sbs_read_u1(uint8_t *data, uint8_t cmd) {
    ...
    int ret_code = bq_handle_i2c_transfer(bq_cmd, rx_buf, 1);
    *data = rx_buf[0];
    return ret_code;
}
...
static int sbs_read_u2(uint16_t *data, uint8_t cmd) {
    ...
    uint8_t bq_cmd[1] = { cmd };
    int ret_code = !bq_handle_i2c_transfer(bq_cmd, rx_buf, 2);
...
static int sbs_read_h4(uint32_t *data, uint8_t cmd) {
    ...
    uint8_t bq_cmd[1] = { cmd };
    int ret_code = !bq_handle_i2c_transfer(bq_cmd, rx_buf, 5);
