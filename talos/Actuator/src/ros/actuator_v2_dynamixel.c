...
rcl_ret_t actuator_v2_dynamixel_init(rcl_node_t *node, rclc_executor_t *executor) {
    RCRETCHECK(rclc_publisher_init_best_effort(&dynamixel_status_publisher, node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(riptide_msgs2, msg, DynamixelStatus), DYNAMIXEL_STATUS_TOPIC_NAME));

    RCRETCHECK(rclc_service_init_default(&move_home_service, node, ROSIDL_GET_SRV_TYPE_SUPPORT(std_srvs, srv, Trigger), TORPEDO_MARKER_MOVE_HOME_SERVICE_NAME));
    RCRETCHECK(rclc_executor_add_service(executor, &move_home_service, &move_home_req, &move_home_res, move_home_service_callback));

    RCRETCHECK(rclc_service_init_default(&set_home_service, node, ROSIDL_GET_SRV_TYPE_SUPPORT(std_srvs, srv, Trigger), TORPEDO_MARKER_SET_HOME_SERVICE_NAME));
    RCRETCHECK(rclc_executor_add_service(executor, &set_home_service, &set_home_req, &set_home_res, set_home_service_callback));
