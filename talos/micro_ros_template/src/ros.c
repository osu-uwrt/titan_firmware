...
rcl_subscription_t channel_subscriber;
std_msgs__msg__Int32 channel_msg;
volatile int32_t latest_channel_value;
...
static void channel_subscription_callback(const void *msgin) {
    const std_msgs__msg__Int32 *msg = (const std_msgs__msg__Int32 *) msgin;
    latest_channel_value = msg->data;

    update_hbridge(latest_channel_value);

    printf("Received channel value %d\n", latest_channel_value);
}
...
rcl_ret_t ros_init() {
    ...
    RCRETCHECK(rclc_subscription_init_default(&channel_subscriber, &node,
                                              ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int32), "channel"));
    ...
    RCRETCHECK(rclc_executor_add_subscription(&executor, &channel_subscriber, &channel_msg,
                                              &channel_subscription_callback, ON_NEW_DATA));
...
void ros_fini(void) {
    ...
    RCSOFTCHECK(rclc_subscription_fini(&channel_subscriber, &node));
