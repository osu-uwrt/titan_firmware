...
#define PINGER_PUBLIS_INTERVAL_MS 1000
...
absolute_time_t next_pinger_publish = { 0 };
...
static void start_ros_timers() {
    ...
    next_pinger_publish = make_timeout_time_ms(PINGER_PUBLIS_INTERVAL_MS);
}
...
static void tick_ros_tasks() {
    ...
    if (timer_ready(&next_pinger_publish, PINGER_PUBLIS_INTERVAL_MS, true)) {
        RCSOFTRETVCHECK(ros_update_pinger1_publisher());
        RCSOFTRETVCHECK(ros_update_pinger2_publisher());
    }
}
...
// Initialize stdio
    ...
    gpio_init(INPUT1_PIN);
    gpio_set_dir(INPUT1_PIN, false);
    gpio_int(INPUT2_PIN);
    gpio_set_dir(INPUT2_PIN, false);
