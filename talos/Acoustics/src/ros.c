...
#define PINGER1_PUBLISHER_NAME "pinger1"
#define PINGER2_PUBLISHER_NAME "pinger2"
...
rcl_publisher_t pinger1_timestamp_publisher;
rcl_publisher_t pinger2_timestamp_publisher;
...
// TODO: Add in node specific tasks here
rcl_ret_t ros_update_pinger1_publisher() {
    std_msgs__msg__UInt32 pinger1_timestamp_msg;
    pinger1_timestamp_msg.data = 1;
    RCSOFTRETCHECK(rcl_publish(&pinger1_timestamp_publisher, &pinger1_timestamp_msg, NULL));
    return RCL_RET_OK;
}
rcl_ret_t ros_update_pinger2_publisher() {
    std_msgs__msg__UInt32 pinger2_timestamp_msg;
    pinger2_timestamp_msg.data = 2;
    RCSOFTRETCHECK(rcl_publish(&pinger2_timestamp_publisher, &pinger2_timestamp_msg, NULL));
    return RCL_RET_OK;
}
...
rcl_ret_t ros_init() {
    ...
    RCRETCHECK(rclc_publisher_init_best_effort(&pinger1_timestamp_publisher, &node,
                                               ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt32),
                                               PINGER1_PUBLISHER_NAME));

    RCRETCHECK(rclc_publisher_init_best_effort(&pinger2_timestamp_publisher, &node,
                                               ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt32),
                                               PINGER2_PUBLISHER_NAME));
...
void ros_fini(void) {
    ...
    RCSOFTCHECK(rcl_publisher_fini(&pinger1_timestamp_publisher, &node));
    RCSOFTCHECK(rcl_publisher_fini(&pinger2_timestamp_publisher, &node));
