...
namespace picoboot {
    ...
    struct connection {
        explicit connection(libusb_device_handle *device, struct rp2040_bootrom_usb_data* usbdata, bool exclusive = true) : device(device), usbdata(usbdata), exclusive(exclusive) {
            if (exclusive) exclusive_access(EXCLUSIVE);
        }
        ~connection() {
            if (exclusive) {
                picoboot_exclusive_access(device, usbdata, NOT_EXCLUSIVE);
            }
        }
        void reset();
