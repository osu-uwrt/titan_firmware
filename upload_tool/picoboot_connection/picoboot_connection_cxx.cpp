...
template <typename F> void connection::wrap_call(F&& func) {
    ...
    if (rc) {
        ...
        rc = picoboot_cmd_status(device, usbdata, &status);
        if (!rc) {
...
void connection::reset() {
    wrap_call([&] { return picoboot_reset(device, usbdata); });
}

void connection::exclusive_access(uint8_t exclusive) {
    wrap_call([&] { return picoboot_exclusive_access(device, usbdata, exclusive); });
}

void connection::enter_cmd_xip() {
    wrap_call([&] { return picoboot_enter_cmd_xip(device, usbdata); });
}

void connection::exit_xip() {
    wrap_call([&] { return picoboot_exit_xip(device, usbdata); });
}

void connection::reboot(uint32_t pc, uint32_t sp, uint32_t delay_ms) {
    wrap_call([&] { return picoboot_reboot(device, usbdata, pc, sp, delay_ms); });
}

void connection::exec(uint32_t addr) {
    wrap_call([&] { return picoboot_exec(device, usbdata, addr); });
}

void connection::flash_erase(uint32_t addr, uint32_t len) {
    wrap_call([&] { return picoboot_flash_erase(device, usbdata, addr, len); });
}

void connection::vector(uint32_t addr) {
    wrap_call([&] { return picoboot_vector(device, usbdata, addr); });
}

void connection::write(uint32_t addr, uint8_t *buffer, uint32_t len) {
    wrap_call([&] { return picoboot_write(device, usbdata, addr, buffer, len); });
}

void connection::read(uint32_t addr, uint8_t *buffer, uint32_t len) {
    wrap_call([&] { return picoboot_read(device, usbdata, addr, buffer, len); });
}
