...
int picoboot_reset(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata) {
    if (verbose) output("RESET\n");
    if (is_halted(usb_device, usbdata->in_ep))
        libusb_clear_halt(usb_device, usbdata->in_ep);
    if (is_halted(usb_device, usbdata->out_ep))
        libusb_clear_halt(usb_device, usbdata->out_ep);
    int ret =
            libusb_control_transfer(usb_device, LIBUSB_REQUEST_TYPE_VENDOR | LIBUSB_RECIPIENT_INTERFACE,
                                    PICOBOOT_IF_RESET, 0, usbdata->interface, NULL, 0, 1000);
...
int picoboot_cmd_status_verbose(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, struct picoboot_cmd_status *status, bool local_verbose) {
    ...
    int ret =
            libusb_control_transfer(usb_device,
                                    ...
                                    PICOBOOT_IF_CMD_STATUS, 0, usbdata->interface, (uint8_t *) status, sizeof(*status), 1000);
...
int picoboot_cmd_status(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, struct picoboot_cmd_status *status) {
    return picoboot_cmd_status_verbose(usb_device, usbdata, status, verbose);
}

int one_time_bulk_timeout;

int picoboot_cmd(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, struct picoboot_cmd *cmd, uint8_t *buffer, uint buf_size) {
    int sent = 0;
    int ret;
    ...
    cmd->dToken = token++;
    ret = libusb_bulk_transfer(usb_device, usbdata->out_ep, (uint8_t *) cmd, sizeof(struct picoboot_cmd), &sent, 3000);
    ...
    if (cmd->dTransferLength != 0) {
        ...
        if (cmd->bCmdId & 0x80u) {
            ...
            int received = 0;
            ret = libusb_bulk_transfer(usb_device, usbdata->in_ep, buffer, cmd->dTransferLength, &received, timeout);
            ...
            ret = libusb_bulk_transfer(usb_device, usbdata->out_ep, buffer, cmd->dTransferLength, &sent, timeout);
            if (ret != 0 || sent != (int) cmd->dTransferLength) {
                output("  ...failed to send data %d %d/%d\n", ret, sent, cmd->dTransferLength);
                if (!ret) ret = 1;
                picoboot_cmd_status_verbose(usb_device, usbdata, NULL, true);
                return ret;
            }
        }
    }
    ...
    if (cmd->bCmdId & 0x80u) {
        ...
        ret = libusb_bulk_transfer(usb_device, usbdata->out_ep, spoon, 1, &received, cmd->dTransferLength == 0 ? timeout : 3000);
    } else {
        if (verbose) output("zero length in\n");
        ret = libusb_bulk_transfer(usb_device, usbdata->in_ep, spoon, 1, &received, cmd->dTransferLength == 0 ? timeout : 3000);
    }
    return ret;
}

int picoboot_exclusive_access(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint8_t exclusive) {
    ...
    return picoboot_cmd(usb_device, usbdata, &cmd, NULL, 0);
}

int picoboot_exit_xip(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata) {
    struct picoboot_cmd cmd;
...
int picoboot_enter_cmd_xip(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata) {
    struct picoboot_cmd cmd;
...
int picoboot_reboot(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t pc, uint32_t sp, uint32_t delay_ms) {
    struct picoboot_cmd cmd;
...
int picoboot_exec(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t addr) {
    struct picoboot_cmd cmd;
...
int picoboot_flash_erase(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t addr, uint32_t len) {
    struct picoboot_cmd cmd;
...
int picoboot_vector(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t addr) {
    struct picoboot_cmd cmd;
...
int picoboot_write(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t addr, uint8_t *buffer, uint32_t len) {
    struct picoboot_cmd cmd;
    ...
    return picoboot_cmd(usb_device, usbdata, &cmd, buffer, len);
}

int picoboot_read(libusb_device_handle *usb_device, struct rp2040_bootrom_usb_data* usbdata, uint32_t addr, uint8_t *buffer, uint32_t len) {
    ...
    cmd.range_cmd.dAddr = addr;
    cmd.range_cmd.dSize = cmd.dTransferLength = len;
    int ret = picoboot_cmd(usb_device, usbdata, &cmd, buffer, len);
