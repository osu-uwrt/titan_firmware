...
static const uint irq_num = (BOARD_I2C == i2c0 ? I2C0_IRQ : I2C1_IRQ);

void async_i2c_irq_handler(void) {
    async_i2c_common_irq_handler(BOARD_I2C);
}
...
void async_i2c_init(uint baudrate, uint bus_timeout_ms) {
    ...
    async_i2c_configure_interrupt_hw(BOARD_I2C);
