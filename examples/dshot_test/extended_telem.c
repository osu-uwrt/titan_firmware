...
void __time_critical_func(telem_missed_irq)(void) {
    //puts("No Telemetry Received");
    pio_interrupt_clear(DSHOT_PIO, 0);
}
...
enum packet_type {
    TELEM_ERPM_PERIOD = 0x00,
    TELEM_TEMPERATURE = 0x02,
    TELEM_CURRENT = 0x06,
    TELEM_DEBUG_1 = 0x08,
    TELEM_DEBUG_2 = 0x0A,
    TELEM_DEBUG_3 = 0x0C,
    TELEM_STATE_EVENT = 0x0E
};

bool __time_critical_func(decode_bidir_telemetry)(uint32_t packet, enum packet_type *type_out, uint32_t *value_out) {
    if (packet == 0) {
        return false;
    }
    ...
    // Check if extended dshot telemetry
    if ((telem_data & 0x100) == 0 && (telem_data >> 8) != 0) {
        *type_out = (telem_data >> 8);
        *value_out = (telem_data & 0xFF);
    } else {
        *type_out = TELEM_ERPM_PERIOD;
        *value_out = (telem_data & 0x1FF) << (telem_data >> 9);
    }

    return true;
}

void handle_packet(int id, enum packet_type type, uint32_t value) {
    if (type == TELEM_ERPM_PERIOD) {
        uint32_t rpm = 0;
        // Check if reporting max period (no RPM)
        if (value != (0x1FF << 0x7) && value != 0) {
            uint32_t erpm = (60 * 1000000) / value;
            ...
            rpm = erpm / num_pole_pairs;
        }
        //printf("RPM %d: %d\n", id, rpm);
    } else {
        printf("EDT %d: %d - %d", id, type, value);
    }
}
...
void __time_critical_func(telem_cb)(void) {
    for (int i = 0; i < 4; i++) {
        if (pio_sm_get_rx_fifo_level(DSHOT_PIO, i) > 0) {
            ...
            uint32_t value;
            if(decode_bidir_telemetry(pio_sm_get_blocking(DSHOT_PIO, i), &type, &value)) {
                handle_packet(i+1, type, value);
            }
        }
    }
...
int main() {
    ...
    while (true) {
        ...
        if (time_reached(send_packet) && !sent) {
            ...
            sent = true;
            for (int i = 0; i < 6; i++) {
                dshot_send_command(0, 33, true);
                sleep_ms(1);
            }
            dshot_send_command(0, 43, true);
            sleep_ms(1);
        }

        gpio_put(STATUS_LEDB_PIN, false);
        if (get_bootsel_button()){
            dshot_send_command(0, 150, false);
        } else {
            dshot_send_command(0, 0, false);
        }
        dshot_send_command(1, 0, false);
        dshot_send_command(2, 0, false);
        dshot_send_command(3, 0, false);
        gpio_put(STATUS_LEDB_PIN, true);
        sleep_ms(1);
    }
    return 0;
}